extends Control
class_name TrackerDisplay

## Class for the Player to track specific Components flowing in an out

var component_goal_slot_scene : PackedScene = load("res://scenes/ui/component_goal_slot.tscn")
var selected_view_i = 0

func _ready():
	Contracts.accepted_contract.connect(_on_contract_accepted)

func _on_contract_accepted(contract: Contract) -> void:
	contract.fulfilled.connect(_on_contract_fulfilled)
	add_new_contract_tracker_view(contract)

func _on_contract_fulfilled(contract: Contract) -> void:
	for i in range(get_child_count()):
		var child = get_child(i)
		if child.name == contract.display_name:
			child.queue_free()
			break
	
	select_next_available_tracker_view()

func add_new_contract_tracker_view(_contract: Contract) -> void:
	pass
	# Add Goal Slots that increment when the associated component is delivered

func select_next_available_tracker_view() -> void:
	if get_child_count() == 0:
		return
	
	get_child(selected_view_i).hide()
	selected_view_i = (selected_view_i + 1) % get_child_count()
	get_child(selected_view_i).show()
